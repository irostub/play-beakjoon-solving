...
class Main:
    ...
    T = int(input())
    for _ in range(T):
        p1.x, p1.y, p1.target_distance, p2.x, p2.y, p2.target_distance = map(
            int, input().split()
        )
        dist = math.sqrt(math.pow(p2.x - p1.x, 2) + math.pow(p2.y - p1.y, 2))
        r_sum, r_sub = p2.target_distance + p1.target_distance, abs(
            p2.target_distance - p1.target_distance
        )
        if dist == 0:
